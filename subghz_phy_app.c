...
// Implement dtostrf function

char* dtostrf(double val, signed char width, unsigned char prec, char* s) {
    int whole = (int)val;
    int frac = (int)((val - whole) * pow(10, prec));

    snprintf(s, width + prec + 2, "%d.%0*d", whole, prec, frac);
    return s;
}
  ...
  else
    {
	  if (id_frame != 0){
		  uint8_t my_appKey[4] = {5, 6, 7, 8};
		             char message[84] = {0};
		             memcpy(data_buffer, my_appKey, app_key_offset);
		             uint8_t r_size;
		             ...
		             snprintf(final_str, sizeof(final_str), "%s!%d!%d", begin, id_node, id_frame++);

		             dtostrf((float)TEMP,2,4,auxTEMP);
		             dtostrf((float)RH,2,4,auxRH);
		             dtostrf((float)LDR,2,4,auxLUX);

		             size_t final_str_len = strlen(final_str);
		             snprintf(final_str + final_str_len, sizeof(final_str) - final_str_len, "#%d/%s#%d/%s#%d/%s", id_temperature, auxTEMP, id_humidity, auxRH, id_LDR, auxLUX);


		             r_size = sprintf(message + app_key_offset, final_str); // uncomment


		             memcpy(data_buffer, (uint8_t *)message, r_size + app_key_offset);
		             delay(10000000);
		             for(int i = 0; i < r_size + app_key_offset; i++) {

		                  //     APP_PPRINTF("%c", data_buffer[i]);
		                  //   APP_PPRINTF("%02X ", data_buffer[i]);
		            }
	  }else{
		  id_frame++;
	  }
